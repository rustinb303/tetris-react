...
const INITIAL_DROPPING_COLUMN = 3;
const MOVEMENT_INTERVAL_MS = 300;
...
// Game speed configurations in milliseconds
enum TickSpeed {
  Normal = 800,
  Sliding = 100,
  Fast = 50,
...
export function useTetris() {
  ...
  const commitPosition = useCallback(() => {
    ...
    if (hasCollisions(board, SHAPES[newBlock].shape, 0, INITIAL_DROPPING_COLUMN)) {
      saveHighScore(score);
      setIsPlaying(false);
      setTickSpeed(null);
    } else {
