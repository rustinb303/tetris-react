...
export function useTetris() {
  ...
  const startGame = useCallback((hardcore = false) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowDown') {
        setTickSpeed(isHardcore ? TickSpeed.Hardcore : TickSpeed.Fast);
      }
  ...
  }, [dispatchBoardState, isPlaying, isHardcore]);
  ...
  return {
    board: renderedBoard,
    startGame,
    startHardcoreGame: () => startGame(true),
    isPlaying,
    score,
    upcomingBlocks,
