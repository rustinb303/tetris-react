...
import { useTetris, saveHighScore, GetHighScores, TickSpeed } from '../hooks/useTetris';
...
describe('useTetris', () => {
  ...
  describe('Hardcore Mode', () => {
    it('toggleHardcoreMode updates isHardcoreMode state', () => {
      const { result } = renderHook(() => useTetris());
    ...
    it('startGame sets tickSpeed to Hardcore if isHardcoreMode is true', () => {
      const { result } = renderHook(() => useTetris());
      act(() => {
        result.current.toggleHardcoreMode(); // Enable hardcore mode
      });
      act(() => {
        result.current.startGame();
      });
    ...
    it('startGame sets tickSpeed to Normal if isHardcoreMode is false', () => {
      const { result } = renderHook(() => useTetris());
      // isHardcoreMode is false by default
      act(() => {
        result.current.startGame();
      });
    ...
    it('toggleHardcoreMode updates tickSpeed during an active game', () => {
      const { result } = renderHook(() => useTetris());
      act(() => {
        result.current.startGame(); // Game starts, speed is Normal
      });
      expect(result.current.tickSpeed).toBe(TickSpeed.Normal);
      ...
      act(() => {
        result.current.toggleHardcoreMode(); // Disable hardcore
      });
      expect(result.current.tickSpeed).toBe(TickSpeed.Normal);
    });

    describe('ArrowDown key behavior with Hardcore Mode', () => {
      it('reverts to Hardcore speed if hardcore is enabled', () => {
        const { result } = renderHook(() => useTetris());
        act(() => result.current.toggleHardcoreMode()); // Enable hardcore
      ...
      it('reverts to Normal speed if hardcore is disabled', () => {
        const { result } = renderHook(() => useTetris());
        // Hardcore is disabled by default
        act(() => result.current.startGame());
        expect(result.current.tickSpeed).toBe(TickSpeed.Normal);
