"""Final working demo script with database integration for CrewAI agents."""
import os
import sys
import json
import re
...
class SimpleDatabaseTool(BaseTool):
    """Simple database tool that works with CrewAI's expected format."""
    
    name: str = "SimpleDatabaseTool"
    description: str = "Tool for reading and writing data to a simple database."
    
    def __init__(self, table_name: str):
        """Initialize the simple database tool."""
        super().__init__()
        self.name = "SimpleDatabaseTool"
...
def run_final_demo(topic="AI trends"):
    """
    Run a demo with agents that can access and manipulate data in a simple database.
    ...
    print(f"Running final demo with topic: {topic}")
    
    print("Setting up simple database...")
    db_tool = SimpleDatabaseTool(table_name="research_data")
    ...
    research_task = Task(
        ...
        To store your findings, use the SimpleDatabaseTool with natural language queries like:
        
        ```
        Action: SimpleDatabaseTool
    ...
    analysis_task = Task(
        ...
        To retrieve the data, use the SimpleDatabaseTool with natural language queries like:
        ```
        Action: SimpleDatabaseTool
